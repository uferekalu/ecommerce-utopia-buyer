...
class ModuleProductHasVariants extends Component {
    ...
    handleSelectColor(colorId) {
        ...
        if (product && product.length > 0) {
            const selectedVariant = product.find(
                (item) => item.id_product_m2m_vendor === colorId
            );
    ...
    render() {
        ...
        if (product) {
            if (product.length > 0) {
                let colorSelectionArea = product.map((item) => {
                    return (
                        <div
                            className={`ps-variant ps-variant--image ${
                                selectedVariant &&
                                selectedVariant.id === item.id_product_m2m_vendor
                                    ? 'active'
                                    : ''
                            }`}
                            key={item.id_product_m2m_vendor}
                            onClick={(e) => this.handleSelectColor(item.id_product_m2m_vendor)}>
