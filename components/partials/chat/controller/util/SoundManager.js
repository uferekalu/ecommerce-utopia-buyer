        ...
        this.enableSoundForMessages(context).then((response) => {
            if (response === true) {
                if (this.incomingMessageAudio === null) {
                    // import('../resources/audio/comingmessage1.wav').then(
                    //     (response) => {
                    //         this.incomingMessageAudio = new Audio(
                    //             response.default
                    //         );
        ...
        this.enableSoundForMessages(context).then((response) => {
            if (response === true) {
                if (this.incomingOtherMessageAudio === null) {
                    // import('../resources/audio/othermessage.wav').then(
                    //     (response) => {
                    //         this.incomingOtherMessageAudio = new Audio(
                    //             response.default
                    //         );
                    //         this.playMessageAlert(
                    //             this.incomingOtherMessageAudio
                    //         );
                    //     }
                    // );
                } else {
        ...
        this.enableSoundForMessages(context).then((response) => {
            if (response === true) {
                if (this.outgoingMessageAudio === null) {
                    // import('../resources/audio/goingmessagesound1.wav').then(
                    //     (response) => {
                    //         this.outgoingMessageAudio = new Audio(
                    //             response.default
                    //         );
                    //         console.log(this.outgoingMessageAudio);
        ...
        this.enableSoundForCalls(context).then((response) => {
            if (response === true) {
                if (this.incomingCallAudio === null) {
                    // import('../resources/audio/call1.wav').then((response) => {
        ...
        this.enableSoundForCalls(context).then((response) => {
            if (response === true) {
                if (this.outgoingCallAudio === null) {
                    // import('../resources/audio/testsound.mp3').then(
                    //     (response) => {
